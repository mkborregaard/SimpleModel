...
# This plots the distribution of a single species on the map
function plot_distribution(speciesname, asm)
    Plots.plot(domain(asm), fill = :grey)
    Plots.plot!(species_raster(asm, speciesname))
end
...
function plot_species(speciesnames, asm)
    pca1, pca2 = asm[!, :pca1], asm[!, :pca2]
    f = Figure(size = (1500, 700))
    a = Axis(f[1,1], aspect = DataAspect())
    b = Axis(f[1,2], aspect = DataAspect())
    Makie.plot!(a, domain(asm), colormap = "Greys")
    Makie.scatter!(b, collect(zip(pca1, pca2)); markersize = 1, color=(:grey, 0.5))
    colsize!(f.layout, 1, Auto(0.73))
    rich = fill(NaN, dims(domain(asm)))

    for (i, species) in enumerate(speciesnames)
        s = occupied(asm, species)
        rich[cellindices(asm)[s]] .= i
        Makie.scatter!(b, pca1[s], pca2[s];
...
function overplot_pca_space(points, color, asm)
    pca1, pca2 = asm[!, :pca1], asm[!, :pca2]
    p = Figure()
...
function overplot_geo_space(points, col, asm)
    f = Figure()
    a = Axis(f[1,1], aspect = DataAspect())
    Makie.plot!(a, domain(asm), colormap = :Greys)
...
function plot_species_pca(speciesname, asm, sigma = 2;
        weightmap = nothing, method = MCDTrim(), cover = 1.0)
    pca1, pca2 = asm[!, :pca1], asm[!, :pca2]
    xs, ys, t = species_trust(asm, speciesname; weightmap, method)
    length(xs) < 2 && return(Plots.scatter(pca1, pca2, ms = 1, mc = :grey, msw = 0, label = "", title = speciesname, aspect_ratio = 1))
    el = fitellipse(asm, speciesname, sigma; weightmap, method, cover)
    kept = t .> 0
    p = Plots.scatter(pca1, pca2, ms = 1, mc = :grey, msw = 0, label = "",
...
function compare_trust_methods(speciesname, asm, methods;
        weightmap = nothing, sigma = 2, cover = 1.0)
    Plots.plot(
        [plot_species_pca(speciesname, asm, sigma; weightmap, method = m, cover) for m in methods]...,
        layout = (1, length(methods)), size = (450 * length(methods), 450), legend = false
    )
end

function plot_ellipse_patches(myel::Int, asm, emp_ellipses, elsize = 2; weightmap = nothing)
    sp = speciesnames(asm)[myel]
    Plots.plot(
        Plots.heatmap(pca_map(asm, :pca1), title = "temp"),
        Plots.heatmap(pca_map(asm, :pca2), title = "precip"),
        Plots.plot(species_raster(asm, sp), title = "actual range"),
        plot_species_pca(sp, asm, elsize; weightmap),
        Plots.plot(map_ellipse(emp_ellipses[myel], asm), title = "full ellipse range"),
        Plots.plot(make_continuous_range(emp_ellipses[myel], asm), title = "sampled patch"),
